    ...
    <div className="grid" style={{gap:16}}>
      <div className="card">
        ...
        <div style={{marginBottom:12}}>
          <label className="small muted">Search discussions</label>
          <input
            className="input"
            value={search}
            onChange={(e)=>setSearch(e.target.value)}
            placeholder="Search by title, site, or content..."
          />
        </div>
          ...
          <div className="card" key={t.id}>
            <div className="row" style={{justifyContent:'space-between'}}>
              ...
              <div className="row" style={{gap:8}}>
                ...
                {user && (t.author === (user.name || user.email)) && (
                  <button className="btnGhost" onClick={()=>deleteThread(t.id)}>Delete</button>
                )}
              </div>
            </div>
            <div className="sep" />
